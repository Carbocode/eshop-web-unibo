...
class NotificationsManager {
  ...
  async fetchNotifications() {
    try {
      const response = await fetch(
        "http://localhost:8000/src/api/notifications/read.php",
        {
          headers: {
  ...
  async checkNewNotifications() {
    try {
      const response = await fetch(
        "http://localhost:8000/src/api/notifications/check_new.php",
        {
          headers: {
            Authorization: `Bearer ${getToken()}`,
          },
  ...
  async toggleNotificationStatus(notificationId) {
    try {
      const response = await fetch(
        "http://localhost:8000/src/api/notifications/update.php",
        {
          method: "POST",
          headers: {
  ...
  async deleteNotification(notificationId) {
    try {
      const response = await fetch(
        "http://localhost:8000/src/api/notifications/delete.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  renderNotifications() {
    ...
    filteredNotifications.forEach((notification) => {
      ...
      markBtn.classList.add(
        notification.read ? "fa-circle-check" : "fa-circle-check"
      );
